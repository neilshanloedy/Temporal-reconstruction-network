...
for(i in country){
  ...
  print(sprintf("doing %s for %s", i, cnt))  
  ...
  if(cnt %in% "contacts_at_home"){
    dataset <- dataset[dataset$cnt_home == T,]
  } else {
    dataset <- dataset[dataset$cnt_home == F,]
  }
  ...
  contact_age$part_age_cat <- factor(contact_age$part_age_cat,
                                     levels = c("Children", "Teens", "Adult", "Elderly"))
  filename <- sprintf('../../data/data_input/POLYMOD/%s/input_polymod_%s_%s.RData', cnt, cnt, i)
